...
inline Eigen::MatrixXf getCovariance(const std::vector<std::vector<float>> &inMat)
{
    if (inMat.empty())
    {
...
inline Eigen::MatrixXf getPearsonCorr(const Eigen::MatrixXf &cov, const std::vector<float> &vars)
{
    if (vars.empty())
    {
